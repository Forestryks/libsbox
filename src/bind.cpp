...
Bind::Bind(const bind_desc *desc) : inside_(desc->inside), outside_(desc->outside), flags_(desc->flags) {}
...
std::pair<fs::path, fs::path> Bind::get_paths(const fs::path &root_dir, const fs::path &work_dir) {
    if (!outside_.is_absolute()) {
        Context::get().die(format("%s is not absolute path", outside_.c_str()));
    }
...
void Bind::mount(const fs::path &root_dir, const fs::path &work_dir) {
    ...
    if (!fs::exists(from, error)) {
        if (optional) return;
        Context::get().die(format("%s not exists", from.c_str()));
    }
    ...
    if (fs::is_directory(from, error)) {
        ...
        if (error) {
            if (optional) return;
            Context::get().die(format("Cannot create dir %s: %s", to.c_str(), error.message().c_str()));
        }
        if (::mount(from.c_str(), to.c_str(), "none", mount_flags, "") < 0) {
            if (optional) return;
            Context::get().die(format("Cannot mount %s: %m", to.c_str()));
        }
        mounted_ = true;
    } else {
        Context::get().die(format("%s is not directory. Currently libsboxd supports only directories, sorry :(", to.c_str()));
    }
    if (error && !optional) {
        Context::get().die(format("Cannot stat %s: %s", from.c_str(), error.message().c_str()));
    }
}
...
void Bind::umount(const fs::path &root_dir, const fs::path &work_dir) {
    ...
    if (::umount(to.c_str()) != 0) {
        Context::get().die(format("Cannot umount %s: %m", to.c_str()));
    }
    mounted_ = false;
}
